...
OIL_EVENTS = [
    {"date": "2026-03-30", "currency": "WTI", "category": "지정학적 리스크", "bucket": "에너지",
     ...
     "fact": "이란을 둘러싼 미국·이스라엘과의 무력 충돌이 확산되는 가운데, WTI 가격이 배럴당 100달러 안팎까지 급등했다(한국석유공사 기준 3월 월평균 91.00달러, 2월 64.52달러 대비 큰 폭 상승).",
    ...
    {"date": "2026-04-24", "currency": "WTI", "category": "시장동향", "bucket": "시장심리",
     ...
     "fact": "WTI가 4월(월평균 98.06달러)과 5월(월평균 98.51달러) 두 달 연속 높은 수준에서 고점권을 형성한 뒤, 6월 월평균 81.79달러로 낮아지며 점진적인 하락 전환이 확인됐다(한국석유공사 기준).",
...
CPI_EVENTS = [
    {"date": "2026-07-02", "currency": "CPI", "category": "물가", "bucket": "물가",
     ...
     "fact": "국가데이터처가 발표한 6월 소비자물가지수(2020=100 기준 119.99)가 전년 동월 대비 3.2% 상승해, 2023년 12월(3.2%) 이후 2년 6개월 만에 가장 높은 상승률을 기록했다(5월 3.1%에 이어 두 달 연속 3%대). 석유류 가격이 전년 동월 대비 24.7% 급등하며 전체 물가 상승률을 0.93%포인트 끌어올렸다.",
     "opinion": "이란전쟁 여파로 인한 국제유가 급등이 시차를 두고 석유류 가격에 본격 반영되며 물가 상승을 이끈 것으로 해석되었다(선행지표인 유가가 후행지표인 CPI에 반영된 사례). 다만 국가데이터처는 환율 상승분의 영향은 6월까지는 본격적으로 나타나지 않았다고 설명해, 환율의 파급 경로는 유가보다 더딘 것으로 나타났다.",
...
def render_event_list(events_subset: pd.DataFrame):
    ...
    for _, row in events_subset.sort_values("date").iterrows():
        ...
        st.markdown(
            ...
            <span class="event-date">{row['date'].strftime('%Y-%m-%d')} · {row['currency']}</span>
            <span class="tag" style="background-color:{color};">{row['category']}</span>
            <span class="tag" style="background-color:{BUCKET_COLORS.get(row['bucket'],'#868e96')};">{row['bucket']}</span>
            <div class="event-title">{row['title']}</div>
            <div class="event-fact">📌 <b>사실</b>: {row['fact']}</div>
            <div class="event-opinion">💬 <b>시장 해석</b>: {row['opinion']}</div>
            <div class="event-source">출처(참고용): {row['source']}</div>
            </div>""",
            unsafe_allow_html=True,
        )
...
def _pair_interpretation(a: str, b: str, r: float) -> str:
    ...
    if pair == frozenset(["WTI", "CPI"]):
        if same_dir:
            return (
                ...
                "유가 상승이 원자재·물류·에너지 수입 비용 상승을 통해 시차를 두고 물가에 반영되는 "
                "경로가 통계적으로도 뒷받침됩니다."
            )
        return (
    ...
    if pair == frozenset(["USD/JPY", "CPI"]):
        ...
        return (
            ...
            "소비자물가 전반에 미치는 직접적 파급 경로는 유가에 비해 간접적인 편입니다."
        )
    ...
    if pair == frozenset(["USD/CNY", "CPI"]):
        ...
        return base + " 다만 유가·국내 수요 등 다른 요인의 영향력이 더 커서, 위안화 단독의 설명력은 제한적인 편입니다."

    if pair == frozenset(["USD/CNY", "WTI"]):
        if same_dir:
            return (
                "중국은 원유 순수입국이어서 유가 상승이 무역수지 부담을 통해 위안화 약세(USD/CNY 상승) "
                "요인으로 작용하는 전형적인 경로가 확인됩니다."
            )
        return (
            "중국은 원유 순수입국이어서 유가 상승이 위안화 약세 요인으로 작용할 수 있는데, 이번 "
            "기간에는 반대로 유가가 오를 때 위안화가 강세(USD/CNY 하락)를 보였습니다. 최근 미 "
            "달러화 전반의 약세 흐름이 유가-위안화 관계보다 더 크게 작용했을 가능성을 시사합니다."
        )
    ...
    if pair == frozenset(["USD/JPY", "WTI"]):
        if same_dir:
            return (
                ...
                "정책 정상화 속도와 엔 캐리트레이드 지속으로 유가 상승이 엔화 강세로 충분히 전이되지 "
                "못한 것으로 해석됩니다."
            )
        return (
...
def render_correlation_section(panel: pd.DataFrame, plot_template, bg_color, text_color, grid_color):
    ...
    for a, b, val in pairs:
        ...
        strength = "매우 강하게" if abs(val) >= 0.7 else ("어느 정도" if abs(val) >= 0.4 else "약하게")
        note = _pair_interpretation(a, b, val)
        line = f"**{a} ↔ {b}** (r = {val:+.2f}): {strength} {direction} 움직이는 관계로 나타납니다."
        if note:
            line += f" {note}"
        if abs(val) >= 0.6 and frozenset([a, b]) in INDIRECT_PAIRS:
            line += (
                " 다만 두 지표 모두 조회 기간 내내 뚜렷한 추세(trend)를 보이는 시계열이라, 직접적인 "
                "인과 경로가 약하더라도 우연히 높은 상관계수(허위상관)가 나타날 수 있다는 점에 "
                "유의해야 합니다."
            )
        lines.append(f"- {line}")
    st.markdown("\n".join(lines))
...
def render_leading_lagging_section(panel: pd.DataFrame, plot_template, bg_color, text_color, grid_color,
    ...
    if qualifying:
        ...
        for col, (lag, corr_val) in qualifying.items():
            ...
            lines.append(
                f"- **{col}**: {lag_desc}에 CPI 등락률과 예상 방향(원가 상승 → 물가 상승)대로 "
                f"연관되는 모습입니다 (r = {corr_val:+.2f})."
            )
        st.markdown("\n".join(lines))
    else:
...
st.markdown(
    ...
    <span class="asof-badge">📅 CPI: {CPI_ASOF_LABEL}</span>""",
    unsafe_allow_html=True,
)
...
# ============================================================================
    ...
    with col1:
        st.markdown(f"""<div class="metric-card"><h3>{df.index[-1]:%Y-%m-%d} 환율</h3>
        <h1>{latest['Close']:.4f}</h1></div>""", unsafe_allow_html=True)
    with col2:
        arrow = "▲" if change >= 0 else "▼"
        color = "#e03131" if change >= 0 else "#1c7ed6"
        st.markdown(f"""<div class="metric-card"><h3>기준일 {base.name:%Y-%m-%d} 대비</h3>
        <h1 style="color:{color};">{arrow} {abs(change):.4f} ({pct_change:+.2f}%)</h1></div>""",
                    unsafe_allow_html=True)
    with col3:
        st.markdown(f"""<div class="metric-card"><h3>MA20 / MA50 ({df.index[-1]:%Y-%m-%d} 기준)</h3>
        <h1 style="font-size:20px;">{latest['MA20']:.4f} / {latest['MA50']:.4f}</h1></div>""",
                    unsafe_allow_html=True)
