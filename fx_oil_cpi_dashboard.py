    ...
    for a, b, val in pairs:
        ...
        st.markdown(
            ...
            <div class="corr-card-header">
                ...
                <span class="corr-badge" style="background-color:{strength_color};">{strength_label}</span>
                <span class="corr-badge" style="background-color:{direction_color};">{direction_label}</span>
                <span class="corr-rval">r = {val:+.2f}</span>
            </div>
            <div class="corr-headline">{strength_label} 강도로 {direction} 움직이는 관계로 나타납니다.</div>
            <div class="corr-detail">{note}</div>
            {caution_html}
            </div>""",
            unsafe_allow_html=True,
        )
    ...
    if qualifying:
        for col, (lag, corr_val) in qualifying.items():
            ...
            st.markdown(
                f"""<div class="corr-card">
                <div class="corr-card-header">
                    <span class="corr-pair">{col} → CPI</span>
                    <span class="corr-badge" style="background-color:{strength_color};">{lag_desc}</span>
                    <span class="corr-badge" style="background-color:#12b886;">예상 방향과 일치</span>
                    <span class="corr-rval">r = {corr_val:+.2f}</span>
                </div>
                <div class="corr-detail">원가 상승 → 물가 상승이라는 <b>예상 방향</b>대로 CPI 등락률과 연관되는 모습입니다.</div>
                </div>""",
                unsafe_allow_html=True,
            )
    else:
